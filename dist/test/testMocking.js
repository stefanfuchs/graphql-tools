...
describe('Mock', function () {
    ...
    it('throws an error if you forget to pass schema', function () {
        chai_1.expect(function () { return mock_1.addMockFunctionsToSchema({}); }).to.throw('Must provide schema to mock');
    });
    it('throws an error if the property "schema" on the first argument is not of type GraphQLSchema', function () {
        chai_1.expect(function () { return mock_1.addMockFunctionsToSchema({ schema: {} }); }).to.throw('Value at "schema" must be of type GraphQLSchema');
    });
    it('throws an error if second argument is not a Map', function () {
        var jsSchema = makeExecutableSchema_1.buildSchemaFromTypeDefinitions(shorthand);
        chai_1.expect(function () {
            return mock_1.addMockFunctionsToSchema({ schema: jsSchema, mocks: ['a'] });
    ...
    it('throws an error if mockFunctionMap contains a non-function thingy', function () {
        ...
        var mockMap = { Int: 55 };
        chai_1.expect(function () {
            return mock_1.addMockFunctionsToSchema({ schema: jsSchema, mocks: mockMap });
    ...
    it('lets you use mockServer for convenience', function () {
        ...
        return mock_1.mockServer(shorthand, mockMap)
            .query(testQuery)
            .then(function (res) {
            chai_1.expect(res.data.returnInt).to.equal(12345);
            chai_1.expect(res.data.returnFloat)
                .to.be.a('number')
                .within(-1000, 1000);
            chai_1.expect(res.data.returnBoolean).to.be.a('boolean');
            chai_1.expect(res.data.returnString).to.be.a('string');
    ...
    it('mockServer is able to preserveResolvers of a prebuilt schema', function () {
        ...
        makeExecutableSchema_1.addResolveFunctionsToSchema(jsSchema, resolvers);
        var testQuery = "{\n      returnInt\n      returnString\n      returnBirdsAndBees {\n        ... on Bird {\n          returnInt\n        }\n        ... on Bee {\n          returnInt\n        }\n      }\n    }";
        var mockMap = {
            Int: function () { return 12345; },
            Bird: function () { return ({ returnInt: function () { return 54321; } }); },
        ...
        return mock_1.mockServer(jsSchema, mockMap, true)
            .query(testQuery)
            .then(function (res) {
            chai_1.expect(res.data.returnInt).to.equal(12345);
    ...
    it('lets you use mockServer with prebuilt schema', function () {
        ...
        return mock_1.mockServer(jsSchema, mockMap)
            .query(testQuery)
            .then(function (res) {
            chai_1.expect(res.data.returnInt).to.equal(12345);
            chai_1.expect(res.data.returnFloat)
    ...
    it('can mock Unions', function () {
        var jsSchema = makeExecutableSchema_1.buildSchemaFromTypeDefinitions(shorthand);
        makeExecutableSchema_1.addResolveFunctionsToSchema(jsSchema, resolveFunctions);
        var mockMap = {
        ...
        return graphql_1.graphql(jsSchema, testQuery).then(function (res) {
            ...
            chai_1.expect(res.data['returnBirdsAndBees']).to.deep.include({
                returnInt: 10,
                returnString: 'aha',
            });
            return chai_1.expect(res.data['returnBirdsAndBees']).to.deep.include({
                returnInt: 10,
                returnEnum: 'A',
            });
        });
    });
    it('can mock Interfaces by default', function () {
        var jsSchema = makeExecutableSchema_1.buildSchemaFromTypeDefinitions(shorthand);
        makeExecutableSchema_1.addResolveFunctionsToSchema(jsSchema, resolveFunctions);
        var mockMap = {
            Int: function () { return 10; },
        ...
        var testQuery = "{\n      returnFlying {\n        ... on Bird {\n          returnInt\n          returnString\n        }\n        ... on Bee {\n          returnInt\n          returnEnum\n        }\n      }\n    }";
        return graphql_1.graphql(jsSchema, testQuery).then(function (res) {
            chai_1.expect(res.data['returnFlying']).to.deep.include({
                returnInt: 10,
                returnString: 'aha',
            });
            return chai_1.expect(res.data['returnFlying']).to.deep.include({
                returnInt: 10,
                returnEnum: 'A',
            });
        });
    });
    it('can support explicit Interface mock', function () {
        var jsSchema = makeExecutableSchema_1.buildSchemaFromTypeDefinitions(shorthand);
        makeExecutableSchema_1.addResolveFunctionsToSchema(jsSchema, resolveFunctions);
        var spy = 0;
        var mockMap = {
            Bird: function (root, args) { return ({
                id: args.id,
                returnInt: 100,
            }); },
            Bee: function (root, args) { return ({
                id: args.id,
                returnInt: 200,
            }); },
            Flying: function (root, args) {
                spy++;
                var id = args.id;
                var type = id.split(':')[0];
                // tslint:disable-next-line
                var __typename = ['Bird', 'Bee'].find(function (r) { return r.toLowerCase() === type; });
        ...
        var testQuery = "{\n      node(id:\"bee:123456\"){\n        id,\n        returnInt\n      }\n    }";
        return graphql_1.graphql(jsSchema, testQuery).then(function (res) {
            chai_1.expect(spy).to.equal(1); // to make sure that Flying possible types are not randomly selected
            chai_1.expect(res.data['node']).to.include({
                id: 'bee:123456',
                returnInt: 200,
            });
        });
    });
    it('can support explicit UnionType mock', function () {
        var jsSchema = makeExecutableSchema_1.buildSchemaFromTypeDefinitions(shorthand);
        makeExecutableSchema_1.addResolveFunctionsToSchema(jsSchema, resolveFunctions);
        var spy = 0;
        var mockMap = {
            ...
            BirdsAndBees: function (root, args) {
                spy++;
                var id = args.id;
                var type = id.split(':')[0];
                return {
                    __typename: ['Bird', 'Bee'].find(function (r) { return r.toLowerCase() === type; }),
                };
            },
        };
        mock_1.addMockFunctionsToSchema({
        ...
        var testQuery = "{\n        node2(id:\"bee:123456\"){\n          ...on Bee{\n            id,\n            returnEnum\n          }\n        }\n      }";
        return graphql_1.graphql(jsSchema, testQuery).then(function (res) {
            chai_1.expect(spy).to.equal(1);
            chai_1.expect(res.data['node2']).to.include({
                id: 'bee:123456',
                returnEnum: 'A',
            });
        });
    });
    it('throws an error when __typename is not returned within an explicit interface mock', function () {
        var jsSchema = makeExecutableSchema_1.buildSchemaFromTypeDefinitions(shorthand);
        makeExecutableSchema_1.addResolveFunctionsToSchema(jsSchema, resolveFunctions);
        var mockMap = {
            Bird: function (root, args) { return ({
        ...
        mock_1.addMockFunctionsToSchema({ schema: jsSchema, mocks: mockMap });
        var testQuery = "{\n        node(id:\"bee:123456\"){\n          id,\n          returnInt\n        }\n      }";
        var expected = 'Please return a __typename in "Flying"';
        return graphql_1.graphql(jsSchema, testQuery).then(function (res) {
    ...
    it('does not mask resolve functions if you tell it not to', function () {
        ...
        var mockMap = {
            RootQuery: function () { return ({
                ...
                returnString: function (root, args) {
    ...
    it('let you mock with preserving resolvers, also when using logger', function () {
        ...
        var jsSchema = makeExecutableSchema_1.makeExecutableSchema({
            typeDefs: [shorthand],
            resolvers: resolvers,
    ...
    it('lets you mock root query fields', function () {
        ...
        var mockMap = {
            RootQuery: function () { return ({
                returnStringArgument: function (o, a) { return a['s']; },
            }); },
        };
        mock_1.addMockFunctionsToSchema({ schema: jsSchema, mocks: mockMap });
    ...
    it('lets you mock a list of specific variable length', function () {
        ...
        var mockMap = {
            RootQuery: function () { return ({
                returnListOfIntArg: function (o, a) {
                    return new mock_1.MockList(a['l']);
                },
            }); },
            Int: function () { return 12; },
        };
    ...
    it('throws an error if the second argument to MockList is not a function', function () {
        chai_1.expect(function () { return new mock_1.MockList(5, 'abc'); }).to.throw('Second argument to MockList must be a function or undefined');
    });
    ...
    it('lets you use arguments in nested MockList', function () {
        ...
        var mockMap = {
            RootQuery: function () { return ({
                returnListOfListOfIntArg: function () {
                    return new mock_1.MockList(2, function (o, a) { return new mock_1.MockList(a['l']); });
                },
            }); },
            Int: function () { return 12; },
        };
    ...
    it('works for a slightly more elaborate example', function () {
        ...
        var jsSchema = makeExecutableSchema_1.buildSchemaFromTypeDefinitions(short);
        ...
        var mockMap = {
            RootQuery: function () { return ({
                ...
                threads: function (o, a) {
                    return new mock_1.MockList(ITEMS_PER_PAGE * a['num']);
                },
            }); },
            Thread: function () { return ({
                name: 'Lorem Ipsum',
                posts: function (o, a) {
                    return new mock_1.MockList(ITEMS_PER_PAGE * a['num'], function (oi, ai) { return ({ id: ai['num'] }); });
                },
            }); },
    ...
    it('works for resolvers returning javascript Dates', function () {
        var typeDefs = "\n    \tscalar Date\n\n      type DateObject {\n        start: Date!\n      }\n\n      type Query {\n      \tdate1: DateObject\n    \t\tdate2: Date\n        date3: Date\n      }\n    ";
        var resolvers = {
            Query: {
                date1: function () { return ({
                    start: new Date('2018-01-03'),
                }); },
                date2: function () { return new Date('2016-01-01'); },
            },
            DateObject: {
                start: function (obj) { return obj.start; },
            },
            Date: {
                __serialize: function (val) { return val.toISOString(); },
                __parseValue: function (val) { return new Date(val); },
                __parseLiteral: function (val) { return new Date(val); },
            },
        };
        var schema = makeExecutableSchema_1.makeExecutableSchema({
            typeDefs: typeDefs,
            resolvers: resolvers,
        });
        mock_1.addMockFunctionsToSchema({
            schema: schema,
            mocks: {
                Date: function () { return new Date('2016-05-04'); },
            },
            preserveResolvers: true,
        });
        var query = "\n    {\n      date1 {\n        start\n      }\n      date2\n      date3\n    }\n    ";
    ...
    it('allows instanceof checks in __resolveType', function () {
